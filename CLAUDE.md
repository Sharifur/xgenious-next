...
| XG-055 | Genius Campaign lead integration | DONE | HIGH | ~60k | Wired into shared `app/api/download-lead/route.ts` (single choke point for all free-software pages, current + future). Upserts contact (`POST /api/v1/contacts`, listId/tagIds) then enrolls into sequence (`POST /api/v1/contacts/{email}/enroll`), fire-and-forget alongside Taskip push. Env: `GENIUS_CAMPAIGN_API_HOST`, `GENIUS_CAMPAIGN_API_KEY`, `GENIUS_CAMPAIGN_LIST_ID`, `GENIUS_CAMPAIGN_SEQUENCE_ID`, `GENIUS_CAMPAIGN_TAG_ID` in `.env.local`. |
| XG-056 | Wire Sentry SDK (geniusDebug) | DONE | HIGH | ~30k | Installed `@sentry/nextjs`, created client/server/edge configs pointing to `debug-ingest.taskip.net`, wrapped `next.config.ts` with `withSentryConfig` + `/monitoring` tunnel route. Vercel env vars needed: `GENIUSDEBUG_API`, `GENIUSDEBUG_PROJECT_ID`, `GENIUSDEBUG_ORG_TOKEN`. Post-build step: `node scripts/upload-sourcemaps.mjs`. |
| XG-057 | geniusDebug sourcemap upload wiring | DONE | HIGH | ~15k | Added `GENIUSDEBUG_API`/`GENIUSDEBUG_PROJECT_ID`/`GENIUSDEBUG_ORG_TOKEN` to `.env.local` (org token supplied by Taskip client, shown once). Wired `postbuild` script in `package.json` → `node scripts/upload-sourcemaps.mjs` so sourcemaps auto-upload after every `next build` (previously manual-only, never actually ran). Same Vercel env vars must be set in Project Settings for prod builds. |
| XG-058 | Fix geniusDebug sourcemap upload — real API contract | DONE | HIGH | ~40k | Our uploader hit a guessed/wrong endpoint (Sentry-SaaS-style path) — never worked. Ported proven implementation from sibling repo `taskip-client` (`/Users/sharifur/localhost/taskip-client`): correct endpoint `{API}/api/{projectId}/releases/{release}/upload`, gzip+base64 batches of 50, debugId-marker injection workaround (webpack only emits `sentry-dbid-<uuid>` runtime marker without `SENTRY_AUTH_TOKEN`, not the `//# debugId=` comment). Added `sourcemaps.inject: true` + `widenClientFileUpload: true` to `next.config.ts` so the marker actually gets emitted. `vercel.json` `buildCommand` now chains `next build && node scripts/upload-sourcemaps.mjs` directly (no longer depends on npm postbuild lifecycle). |

| XG-059 | geniusDebug upload: retry + non-fatal on 5xx | DONE | HIGH | ~20k | Vercel deploy died on a transient `500` from geniusDebug's upload API mid-batch. Verified against actual API source (`/Users/sharifur/localhost/genius-debug` `admin.controller.ts:519 uploadSourceMaps`) — our payload/contract is correct; handler runs 24 concurrent R2 `PutObject`s per batch and an R2 hiccup throws unhandled → generic 500. `taskip-client`'s script has the same gap (no retry), just hadn't hit it yet. Added 3x retry w/ backoff on 5xx/network errors in `scripts/upload-sourcemaps.mjs`, skip (not throw) after exhausting retries, and the script never exits non-zero — sourcemap upload is best-effort, must never block prod deploy. |

### Sprint Stats
- Total: 59  /  TODO: 0  /  IN_PROGRESS: 0  /  DONE: 59  /  BLOCKED: 0
