...
const CAPS = [
  {
    ...
    desc: 'Enable multiple specialised AI agents to work together, share context, divide tasks, and complete complex business operations collaboratively — with a coordinator agent managing the pipeline.',
  },
  {
    icon: <DatabaseIcon />,
    title: 'Real-Time Decision Engine',
    desc: 'AI agents that analyse live data, understand context, and make intelligent decisions instantly — across workflows, approval queues, and operational systems — without waiting for a human trigger.',
  },
  {
    icon: <GearIcon />,
    title: 'Seamless Tool Integration',
    desc: 'Connect AI agents to your CRM, database, payment processor, ticketing system, and any REST/GraphQL API. Agents read, write, and act inside your existing tools — not alongside them.',
  },
];
...
function CapabilityCard({ icon, title, desc, className = '' }: (typeof CAPS)[number] & { className?: string }) {
  return (
    <div className={`relative mt-[30px] sm:mt-0 ${className}`}>
      ...
      <div
        className="rounded-[16px] p-5 sm:p-[40px] pt-10 sm:pt-14 text-center flex flex-col gap-4"
        style={{ background: '#111111', border: '1px solid rgba(255,255,255,0.08)' }}
      >
        <h3 className="text-white font-medium text-[18px] leading-[26px] sm:text-[20px] sm:leading-[28px] lg:text-[22px] lg:leading-[30px]">{title}</h3>
        <p className="text-[#9ca3af] text-[13px] leading-[20px] sm:text-[14px] sm:leading-[22px]">{desc}</p>
      </div>
    </div>
  );
}
      ...
      <div className="container-page px-4 sm:px-6 lg:px-0 flex flex-col gap-10 sm:gap-16 relative z-10">
        ...
        <div className="flex flex-col items-center text-center gap-5">
          ...
          <p className="text-[#9ca3af] text-[14px] sm:text-[16px] leading-6 max-w-[560px]">
            A complete ecosystem of intelligent AI capabilities designed to automate workflows, enhance decision-making, and scale business operations without scaling headcount.
          </p>
        </div>
