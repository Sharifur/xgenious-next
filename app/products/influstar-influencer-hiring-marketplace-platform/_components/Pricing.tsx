...
const INSTALL_CHECKOUT_URL = `/checkout?product=${INSTALL_PRODUCT_PATH}`;
...
export default function Pricing() {
  return (
    <section id="pricing" className="py-16 sm:py-20 lg:py-[100px]" style={{ background: '#faf5ff' }}>
      <div className="container-page px-4 sm:px-6 lg:px-0">
        ...
        <div className="text-center mb-12 max-w-[620px] mx-auto">
          ...
          <p className="text-[#4b5563] text-[15px] sm:text-[17px] leading-7">
            Buy once and own the full source code. Lifetime updates on every license — no monthly fees, no revenue share.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-[1100px] mx-auto items-stretch">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EC] p-7 flex flex-col">
            <div className="mb-6">
              ...
              <p className="text-[13px] text-[#6b7280] leading-5">Single-project use. Purchase directly on CodeCanyon.</p>
            </div>

            <CodeCanyonUpsellButton
              codecanyonUrl={PURCHASE_URL}
              regularPrice={REGULAR_PRICE}
              bundlePrice={INSTALL_PRICE}
              bundleCheckoutUrl={INSTALL_CHECKOUT_URL}
              bundleItems={['Professional installation on your server', 'Server & database configuration', 'Ready in 48 hours']}
              bundleLabel="Regular + Installation"
              accentColor={COLOR}
          ...
          <div className="rounded-2xl p-7 flex flex-col relative overflow-hidden" style={{ background: COLOR_DARK, border: `2px solid ${COLOR}` }}>
            <div className="flex items-center gap-2 flex-wrap mb-5">
              ...
              <span className="text-[11px] font-bold px-3 py-1 rounded-full" style={{ background: '#3b1d6e', color: '#fff' }}>
                INSTALL INCLUDED
              </span>
            </div>

            <div className="mb-4">
              <p className="text-[13px] font-semibold uppercase tracking-widest mb-3" style={{ color: `${COLOR}cc` }}>Regular + Installation</p>
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-[56px] font-bold text-white leading-none">${INSTALL_PRICE}</span>
                <span className="text-[15px]" style={{ color: '#c4b5fd' }}>one-time</span>
              </div>
              <p className="text-[13px] leading-5 mb-1" style={{ color: '#d8b4fe' }}>
                Full license with <strong className="text-white">professional installation on your server</strong> — skip the setup entirely.
              </p>
            </div>

            <Link
              href={INSTALL_CHECKOUT_URL}
            ...
            <div className="flex flex-col gap-2.5">
              {CORE.map((f) => (
                <div key={f} className="flex items-start gap-2.5 text-[13px]" style={{ color: '#d8b4fe' }}>
                  <CheckIcon color={COLOR} />
                  {f}
                </div>
              ))}
              <div className="flex items-start gap-2.5 text-[13px]" style={{ color: '#d8b4fe' }}>
                <CheckIcon color={COLOR} />
                <span><strong className="text-white">Professional installation</strong> on your server</span>
              </div>
              <div className="flex items-start gap-2.5 text-[13px]" style={{ color: 'rgba(216,180,254,0.55)' }}>
                <XIcon />
          ...
          <div className="rounded-2xl p-7 flex flex-col" style={{ background: PURPLE_DARK, border: `1px solid ${PURPLE}40` }}>
            <div className="mb-5">
              ...
              <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: '#a78bfa' }}>
              ...
              <div className="rounded-xl p-3.5 mb-5" style={{ background: '#150a2e', border: `1px solid ${PURPLE}30` }}>
                ...
                <div className="flex flex-col gap-2.5">
                  {[
                    ...
                    'Installation included ($99 value)',
            ...
            <div className="flex flex-col gap-2.5">
              {[...CORE, 'Professional installation — included ($99 value)', 'Full source code modification rights', 'Remove / replace all branding', 'Priority support — 6 months'].map((f) => (
        ...
        <p className="text-center text-[13px] text-[#9ca3af] mt-8">
          All licenses include lifetime updates · Full Laravel 12 source code · Prices in USD
        </p>
      </div>
    </section>
  );
