...
export default function Comparison() {
  return (
    <section className="py-14 sm:py-20" style={{ background: '#F8FBF8' }}>
      <div className="container-page px-4 sm:px-6 lg:px-0">
        ...
        <div className="text-center mb-10 max-w-[680px] mx-auto">
          <h2 className="text-[28px] sm:text-[40px] font-bold text-[#0F1112] mb-3">
            Grenmart vs Shopify vs WooCommerce vs Custom Build
          </h2>
          <p className="text-[#6b7280] text-[15px] leading-6">
            With Grenmart you <strong>own the code and pay once</strong>. Shopify rents you a store with monthly fees;
            WooCommerce needs paid plugins to match these features; a custom build costs months and five figures.
          </p>
        </div>
          ...
          <table className="w-full min-w-[760px] text-[14px]">
            <thead>
              <tr style={{ background: DARK_BG }}>
                ...
                <th className="px-5 py-4 text-[13px] font-bold" style={{ color: '#7ee29a' }}>Grenmart</th>
            ...
            <tbody>
              {COMPARISON_ROWS.map((row, i) => (
                <tr key={row.feature} style={{ background: i % 2 === 0 ? '#fff' : '#F8FBF8' }}>
                  <td className="px-5 py-3.5 font-medium text-[#374151]">{row.feature}</td>
                  <td className="px-5 py-3.5 text-center font-semibold" style={{ color: highlight(row.grenmart) ? COLOR_DARK : '#374151' }}>
                    {row.grenmart}
                  </td>
