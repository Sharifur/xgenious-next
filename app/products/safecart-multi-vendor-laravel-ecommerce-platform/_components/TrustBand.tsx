...
const STATS = [
  { value: '402+',     label: 'Sales on CodeCanyon' },
  { value: '3.36/5',  label: '22 verified reviews' },
  { value: '26+',     label: 'Payment gateways' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-[1100px] mx-auto">
          ...
          <div>
            <h2 className="text-[28px] sm:text-[40px] font-bold text-[#0F1112] mb-4 leading-tight">
              Test the Live Demo Before You Buy
            </h2>
            <p className="text-[#6b7280] text-[15px] leading-7 mb-8">
              SafeCart has a full live demo with open admin and vendor access. Browse the storefront as a customer, manage
              products as a vendor, and configure the marketplace as an admin — then buy with confidence knowing
              exactly what you get. Every license ships with the full Laravel source code and lifetime updates.
            </p>
            ...
            <div className="col-span-2 bg-white rounded-2xl border border-[#E5E7EC] p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <Link href={PURCHASE_URL} target="_blank" rel="noopener noreferrer" className="text-[12px] text-[#6b7280] hover:underline">
                  Sold on CodeCanyon · 400+ customers
                </Link>
              </div>
            </div>
          </div>
