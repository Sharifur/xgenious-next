...
export default function Gateways() {
  return (
    <section className="py-16 sm:py-20 lg:py-[100px] overflow-hidden bg-white">
      <style>{`
        @keyframes zaika-marquee-left  { from { transform: translateX(0) }    to { transform: translateX(-50%) } }
        @keyframes zaika-marquee-right { from { transform: translateX(-50%) } to { transform: translateX(0) }    }
        .zaika-marquee-left  { animation: zaika-marquee-left  50s linear infinite; }
        .zaika-marquee-right { animation: zaika-marquee-right 50s linear infinite; }
        .zaika-marquee-wrap:hover .zaika-marquee-left,
        .zaika-marquee-wrap:hover .zaika-marquee-right { animation-play-state: paused; }
      `}</style>
      ...
      <div className="zaika-marquee-wrap flex flex-col gap-4 mb-16">
        {/* Row 1 — scrolls left */}
        <div className="flex w-max zaika-marquee-left">
          {[...row1, ...row1, ...row1, ...row1].map((logo, i) => (
            <LogoCard key={i} {...logo} />
          ))}
        </div>
        {/* Row 2 — scrolls right */}
        <div className="flex w-max zaika-marquee-right">
