...
export async function DELETE() {
  ...
  const res = await lsFetch('/envato/connections', {
    method: 'DELETE',
  ...
  if (!res.ok) {
    console.error('[license-server] envato disconnect error:', res.status, JSON.stringify(data));
    const userMsg = res.status < 500
      ? ((data as { message?: string }).message ?? 'Failed to disconnect Envato account')
