...
export default function JpgToAvif() {
  ...
  function handleFile(f: File) {
    if (!f.type.includes('jpeg') && !f.type.includes('jpg')) { setError('Please upload a JPG/JPEG file.'); return; }
  ...
  return (
    <div className="flex flex-col gap-5">
      <div
        ...
        className="border-2 border-dashed border-[#E5E7EC] rounded-2xl p-10 flex flex-col items-center gap-3 cursor-pointer hover:border-[#ef4444] transition-colors"
      >
        ...
        {file ? (
          ...
          <div className="text-center">
            <p className="text-[14px] font-medium text-[#0F1112]">Drop JPG here or click to upload</p>
            <p className="text-[12px] text-[#9ca3af] mt-1">JPG / JPEG files only · Max {MAX_MB}MB · AVIF up to 50% smaller than JPG</p>
          </div>
        )}
      </div>
      ...
      {file && (
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[12px] text-[#9ca3af]">AVIF 70% ≈ JPG 90% visually</span>
          </div>
          <input type="range" min={1} max={100} value={quality} onChange={(e) => setQuality(+e.target.value)} className="w-full accent-[#ef4444]" />
        </div>
      )}
