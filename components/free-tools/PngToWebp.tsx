  ...
  return (
    <div className="flex flex-col gap-5">
      <div
        ...
        className="border-2 border-dashed border-[#E5E7EC] rounded-2xl p-10 flex flex-col items-center gap-3 cursor-pointer hover:border-[#3b82f6] transition-colors"
      >
        ...
        {file ? (
          ...
          <div className="text-center">
            ...
            <p className="text-[12px] text-[#9ca3af] mt-1">PNG files only · Max {MAX_MB}MB</p>
          </div>
        )}
      </div>
      ...
      {file && (
        <div className="flex flex-col gap-2">
          ...
          <input type="range" min={1} max={100} value={quality} onChange={(e) => setQuality(+e.target.value)} className="w-full accent-[#3b82f6]" />
        </div>
      )}
        ...
        <div className="rounded-2xl border border-[#E5E7EC] p-6 flex flex-col gap-4">
          <div className="grid sm:grid-cols-3 gap-3 text-center">
            {[
              ...
              { label: 'WebP Size', val: fmtBytes(result.convertedSize), color: COLOR },
