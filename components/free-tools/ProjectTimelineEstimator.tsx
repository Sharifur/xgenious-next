    ...
    <div className="flex flex-col gap-5">
              ...
              <tr key={i} className="border-b border-[#F5F6F8]">
                ...
                <td className="py-2 pr-2">
                  <input type="text" value={t.name} onChange={(e) => update(i, 'name', e.target.value)}
                    className="w-36 outline-none text-[14px] text-[#0F1112] border-b border-transparent focus:border-[#ec7161]" />
                </td>
                {(['optimistic', 'likely', 'pessimistic'] as const).map((f) => (
                  <td key={f} className="py-2 pr-2">
                    <input type="number" value={tasks[i][f]} onChange={(e) => update(i, f, e.target.value)} min={0}
                      className="w-14 text-right outline-none text-[14px] text-[#0F1112] border-b border-[#E5E7EC] focus:border-[#ec7161]" />
                  </td>
                ))}
                ...
                <td className="py-2 pr-2">
                  <input type="text" value={t.dependency} onChange={(e) => update(i, 'dependency', e.target.value)}
                    placeholder="e.g. 1,2"
                    className="w-16 outline-none text-[13px] text-[#6b7280] border-b border-[#E5E7EC] focus:border-[#ec7161]" />
                </td>
                <td className="py-2">
      ...
      <div className="flex gap-3 items-center">
        <button onClick={addTask}
          className="text-[13px] font-medium border border-dashed border-[#E5E7EC] rounded-lg px-4 py-2 text-[#484848] hover:border-[#3b82f6] hover:text-[#3b82f6] active:scale-95 transition-colors">
          + Add Task
        </button>
        <div className="flex items-center gap-2">
          ...
          <input type="number" value={buffer} onChange={(e) => setBuffer(e.target.value)} min={0} max={100}
            className="w-16 rounded-lg border border-[#E5E7EC] bg-white px-3 py-1.5 text-[14px] text-[#0F1112] outline-none focus:border-[#3b82f6] focus:ring-2 focus:ring-[#3b82f6]/20 transition-colors" />
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        {[
          ...
          { label: 'With Buffer', val: `${result.withBuffer.toFixed(1)} days`, sub: `+${buffer}% risk buffer`, color: '#6366f1' },
          { label: '90% Confidence', val: `${result.p90.toFixed(1)} days`, sub: 'statistical upper bound', color: '#f59e0b' },
